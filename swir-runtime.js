...
(() => {
  ...
  const filesystem = Object.freeze({
    ...
    async pickFile(options = {}) {
      return call('filesystem', 'pickFile', [options], async () => {
    ...
    async pickDirectory(options = {}) {
      return call('filesystem', 'pickDirectory', [options], async () => {
  ...
  const network = Object.freeze({
    async status() {
      return call('network', 'status', [], async () => ({
        online: navigator.onLine,
        type: navigator.connection?.type || navigator.connection?.effectiveType || 'unknown',
    ...
    scan: (...args) => call('network', 'scan', args),
    connect: (...args) => call('network', 'connect', args),
    disconnect: (...args) => call('network', 'disconnect', args)
  });
  ...
  function capabilities() {
    const native = !!host();
    const surfaces = ['filesystem','processes','clipboard','tray','network','updater'];
    const result = {};
    for (const surface of surfaces) {
      const impl = host()?.[surface];
      result[surface] = {
        provider: impl ? 'native' : 'web',
        native: !!impl,
        methods: impl ? Object.keys(impl).filter(k => typeof impl[k] === 'function') : Object.keys(api[surface] || {}).filter(k => typeof api[surface][k] === 'function')
      };
    }
