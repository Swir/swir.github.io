/* SWIR Install Pipeline 1.0 — VERIFY → TRUST → RESOLVE → PERMISSIONS → INSTALL */
(() => {
  'use strict';
  ...
  function sameCatalogPackage(manifest) {
    ...
    return catalog().some(item => item.id === manifest.id && item.packageId === manifest.packageId && String(item.version) === String(manifest.version) && item.entry === manifest.entry);
  }
  ...
  async function prepare(manifest, options = {}) {
    ...
    const stages = [
      stage('INTEGRITY', integrityPlan.manifest?.ok !== false && integrityPlan.entry?.ok !== false, integrityPlan.manifest?.state === 'MISMATCH' || integrityPlan.entry?.ok === false ? 'BLOCKED' : 'VERIFIED', integrityPlan),
  ...
  async function install(manifest, options = {}) {
    ...
    const approvedPermissions = Array.isArray(options.approvedPermissions) ? options.approvedPermissions.map(String) : requested;
    ...
    await api.packages.install({
      ...
      verification:{ schema:SCHEMA, checkedAt:Date.now(), signed:plan.signed, officialCatalog:plan.officialCatalog },
      installed:true
    });
  ...
  async function remove(manifest, options = {}) {
    ...
    return { ok:true, schema:SCHEMA, packageId:manifest.packageId || manifest.id, state:'REMOVED' };
  }

  function info() {
    return { schema:SCHEMA, version:'1.0.0', stages:[...STAGES], integrity:!!integrity(), trust:!!globalThis.SwirTrustedKeys, resolver:!!resolver(), platform:!!platform() };
  }

  globalThis.SwirInstallPipeline = Object.freeze({
    meta:Object.freeze({ name:'SWIR Install Pipeline', version:'1.0.0', schema:SCHEMA, flow:STAGES.join(' → ') }),
    prepare, install, remove, info, sameCatalogPackage
  });
})();
