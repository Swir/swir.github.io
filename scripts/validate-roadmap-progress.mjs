...
for(const required of [marker,start,end,'## 📊 Overall progress','alt="CI"','alt="Roadmap progress"','alt="Completed"','alt="Status"']){
...
for(const expected of exact){if(!progressBlock.includes(expected))fail(`dashboard is stale; expected: ${expected}`)}

const statusMatch=progressBlock.match(/https:\/\/img\.shields\.io\/badge\/STATUS-([^"?]+)-7c3aed\?style=for-the-badge/);
if(!statusMatch)fail('STATUS badge does not match SWIR Roadmap Standard v1');
