...
const checks = [
  ...
  [bridge.includes('BindAuthorizationToBundle(trust, plan)'), 'shipping install binds verified catalog authorization to selected bundle identity'],
  [bridge.includes('CATALOG_PACKAGE_IDENTITY_MISMATCH'), 'signed catalog identity mismatch fails closed before package mutation'],
  ...
  [runtime.includes("version: '1.6.0'"), 'runtime contract version is 1.6.0'],
  ...
  [runtime.includes('function catalogAuthorization(packageId, version, catalog, envelope)'), 'runtime builds structured signed catalog authorization'],
  [runtime.includes('function packageTrustInput(value)'), 'runtime serializes structured trust input at native boundary'],
  [runtime.includes('installAuthorizedFromCapability:'), 'runtime exposes signed catalog capability install helper'],
  [runtime.includes("JSON.stringify(catalogAuthorization(packageId, version, catalog, envelope))"), 'signed install helper serializes authorization rather than trusting a UI hash'],
