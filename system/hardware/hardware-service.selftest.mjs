...
try {
  const pci = path.join(root, 'bus', 'pci', 'devices', '0000:01:00.0');
  ...
  assert.equal(snapshot.schema, 'swir.hardware-snapshot/0.1');
  ...
  assert.throws(() => assertReadOnlyContract({ schema: 'swir.hardware-snapshot/0.1', host: { readOnly: false }, devices: [] }));
