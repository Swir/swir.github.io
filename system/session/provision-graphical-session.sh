...
for pkg in greetd weston plymouth plymouth-themes wayland-utils dbus-user-session cog; do
...
for file in /usr/sbin/greetd /usr/sbin/agreety /usr/bin/weston /usr/bin/wayland-info /usr/bin/plymouth /usr/sbin/plymouth-set-default-theme /usr/bin/cog; do
...
install -d -m 0755 \
  ...
  "$(safe_target /usr/share/swir-shell)" \
...
SHELL_DEST="$(safe_target /usr/share/swir-shell)"
find "$SHELL_DEST" -mindepth 1 -maxdepth 1 -type f -delete
SHELL_ASSETS=0
while IFS= read -r -d '' src; do
  [[ -f "$src" && ! -L "$src" ]] || { echo "refusing non-regular shell asset: $src" >&2; exit 73; }
  base="$(basename "$src")"
  install -m 0644 "$src" "$SHELL_DEST/$base"
  SHELL_ASSETS=$((SHELL_ASSETS + 1))
done < <(find "$SOURCE_ROOT" -maxdepth 1 -type f \( \
  -name 'swir-*.html' -o -name 'swir-*.js' -o -name 'swir-*.css' -o \
  -name 'swir-icon.svg' -o -name 'manifest.webmanifest' \
\) -print0 | sort -z)

[[ $SHELL_ASSETS -ge 20 ]] || { echo "SWIR shell bundle unexpectedly small: $SHELL_ASSETS assets" >&2; exit 70; }
[[ -f "$SHELL_DEST/swir-desktop.html" && ! -L "$SHELL_DEST/swir-desktop.html" ]] || { echo "SWIR shell entry was not staged" >&2; exit 70; }
(
  cd "$SHELL_DEST"
  find . -maxdepth 1 -type f ! -name '.swir-integrity.sha256' -printf '%P\0' | sort -z | xargs -0 sha256sum > .swir-integrity.sha256
  sha256sum -c --strict .swir-integrity.sha256 >/dev/null
)
chown -R root:root "$SHELL_DEST"
chmod 0755 "$SHELL_DEST"
find "$SHELL_DEST" -maxdepth 1 -type f -exec chmod 0644 {} +

cat > "$(safe_target /usr/share/wayland-sessions/swir.desktop)" <<'EOF_SESSION'
[Desktop Entry]
Name=SWIR OS
...
if [[ "$MODE" == production ]]; then
  cat > "$(safe_target /etc/greetd/config.toml)" <<'EOF_GREETD'
[terminal]
vt = 7
...
echo "SWIR graphical session staged: mode=$MODE theme=swir login=greetd compositor=weston shell=cog assets=$SHELL_ASSETS"
