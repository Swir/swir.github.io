...
The `systemd-session-manager` gate proves only that the trusted client required by the separate `SystemSessionIdentityService` is present in the image. It does not fabricate an active graphical user and it does not authenticate or authorize a caller. Session attribution remains a logind concern and privilege remains a separate Polkit/peer-credential concern.

A green readiness result means only that the host contains the prerequisites for the next E2E stage. It does **not** prove final bootability, graphical login, installer correctness, physical hardware support, Secure Boot, successful package mutations or recovery.

## Why this matters

The roadmap requires selecting a maintained Linux base from evidence rather than preference. This gate gives disposable rootfs/VM jobs a stable machine-readable preflight while keeping package, session, security and networking prerequisites explicit rather than inferred.
