...
assert.deepEqual(invocations[0].args, ['--user', 'remotes', '--columns=name,gpg-verify']);
...
const missingRemote = new GuardedFlatpakUserExecutor({ allowlistedRemotes: ['flathub'], fileProbe: trustedBinary, runner: () => ({ status: 0, stdout: 'other\ttrue\n', stderr: '' }) });
assert.throws(() => missingRemote.probe('flathub'), error => error?.code === 'FLATPAK_REMOTE_NOT_CONFIGURED');
const unsignedRemote = new GuardedFlatpakUserExecutor({ allowlistedRemotes: ['flathub'], fileProbe: trustedBinary, runner: () => ({ status: 0, stdout: 'flathub\tfalse\n', stderr: '' }) });
...
const failedExecutor = new GuardedFlatpakUserExecutor({
  ...
  runner(binary, args, options) {
    calls += 1;
    if (calls === 1) return { status: 0, stdout: 'flathub\ttrue\n', stderr: '' };
...
console.log('Flatpak user Package Provider runtime-trust self-test: OK');