...
const requiredTrue = ['directKernelBoot', 'guestNetworkDisabled', 'systemdBooted', 'logindActive', 'networkManagerActive', 'peerAuthorizationSocketActive', 'readinessPassed', 'wineRuntimeRegistryPassed', 'hardwareServicePassed'];
...
if (report?.hardwareInventoryProvider !== 'linux-sysfs') throw new Error('production Hardware Service sysfs provider was not proven');
if (!Number.isInteger(report?.hardwareDevicesObserved) || report.hardwareDevicesObserved < 1) throw new Error('hardware device evidence is missing');
if (!Number.isInteger(report?.hardwareLoadedDriversObserved) || report.hardwareLoadedDriversObserved < 1) throw new Error('bound Linux driver evidence is missing');
