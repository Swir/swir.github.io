...
export function evaluateSystemImageReadiness(evidence) {
  ...
  const gates = [
    ...
    gate('systemd-logind-client', Boolean(selected('sessionManager')?.trusted), selected('sessionManager')?.realPath || 'missing'),
  ...
  const sessionReady = gates.find(item => item.id === 'systemd-logind-client')?.passed === true;
...
export const SystemImageReadinessPolicy = Object.freeze({
  ...
  requiredCore: ['linux-host', 'distribution-identity', 'kernel-visible', 'procfs', 'sysfs', 'trusted-package-manager', 'systemd-service-manager', 'systemd-logind-client', 'polkit-broker', 'networkmanager-client', 'catalog-trust-state-root'],
